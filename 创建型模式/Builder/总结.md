...
abstract class Decorator <<抽象建造者>> {
 - Parlour product
{abstract} +buildWall()
{abstract} +buildTV()
{abstract} +buildSofa()
...
class ConcreteDecorator1 <<具体装修师1>> {
 +buildWall()
 +buildTV()
 +buildSofa()
}

class ConcreteDecorator2 <<具体装修师2>> {
 +buildWall()
 +buildTV()
 +buildSofa()
}

class ProjectManager <<指挥者>> {
 - Decorator builder
...
###建造者模式扩展
建造者（Builder）模式在应用过程中可以根据需要改变，如果创建的产品种类只有一种，只需要一个具体建造者，这时可以省略掉抽象建造者，甚至可以省略掉指挥者角色。这个就类似我们经常写的builder的链式调用。
